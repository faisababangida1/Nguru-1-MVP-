...
app.get('/api/admin/send-nudges', async (req, res) => {
    try {
        ...
        for (let user of usersRes.rows) {
            ...
            const emailContent = `
                <h2>Hey ${cleanName}, Rentra AI here.</h2>
            ...
            const resendResponse = await fetch('https://api.resend.com/emails', {
                ...
                body: JSON.stringify({
                    from: 'Rentra Mentorship <faisal@nguru.online>', 
                    to: user.email,
...
app.listen(PORT, async () => {
    console.log(`Rentra Gateway is live on port ${PORT}`);
    try {
        await pool.query('SELECT 1');
        console.log("Rentra Database Matrix Initialized.");
    } catch (dbErr) {
